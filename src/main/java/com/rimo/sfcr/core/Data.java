...
public class Data {
	...
	public boolean updateWeather(World world) {
		// Weather Pre-detect
		WorldInfo worldProperties = world.getWorldInfo();
		...
		if (worldProperties.isRaining()) {
			...
			int clearWeatherTime = worldProperties.getCleanWeatherTime();
	...
	public void updateDensity(EntityPlayer player) {
		...
		if (enableDynamic) {
			...
			if (! biomeDensityByChunk) {		//Hasn't effected if use chunk data.
				...
				if (CONFIG.isFilterListHasBiome(world.getBiome(pos).getBiomeName()))
					targetDownFall = world.getBiome(pos).getRainfall();
		...
		if (CONFIG.isEnableDebug() && (isWeatherChange || isBiomeChange))
			player.sendStatusMessage(new TextComponentString("[SFCRe Debug] wc:" + isWeatherChange + ", bc:" + isBiomeChange + ", wd:" + densityByWeather + ", bd:" + densityByBiome), true);
	}

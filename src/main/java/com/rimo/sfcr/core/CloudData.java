...
public class CloudData {
	...
	public static void initSampler(long seed) {
		cloudNoise = new SimplexNoise(seed);
	}

	public void tick() {
		FrameTimer frameTimer = Minecraft.getMinecraft().getFrameTimer();
		lifeTime -= (float) (frameTimer.getFrames()[frameTimer.getLastIndex()] / 1000000F * 0.25 * 0.25);
	}
	...
	boolean isCloudCovered(double x, double y, double z) {
		...
		int gx = (int) (width / 2F - (renderManager.viewerPosX - x) / cbSize);
		int gy = (int) (y / cbSize * 2);
		int gz = (int) (width / 2F - (renderManager.viewerPosZ - z) / cbSize);
	...
	private void collectCloudData(int x, int z, float densityByWeather, float densityByBiome) {
		...
		final double time = world.getTotalWorldTime() / 20.0;
		...
		float densityMultiplier = isEnableDynamic ? getDensityMultiplier(world.getWorldTime()) : 1;
		...
		for (int cx = 0; cx < width; cx++) {
			for (int cz = 0; cz < width; cz++) {
				...
				final int h = world.getHeight(bx, bz);  //height sampling use to get biome

				// calculating density...
				if (isEnableDynamic && isBiomeByChunk) {
					BlockPos pos;
					if (isBiomeUseLoadedChunk) {
						Vec3d cellPos = new Vec3d(bx, bz, 0);
						Vec3d unit = new Vec3d(cellPos.x, cellPos.y, 0);
						unit.normalize();
						unit.scale(16);  //measure as chunk
						while (!world.isChunkGeneratedAt((int) cellPos.x / 16, (int) cellPos.y / 16)
								&& unit.dotProduct(cellPos) > 0)  //end when cellPos was reversed.
							cellPos.add(- unit.x, - unit.y, 0);  // stepping pos near towards to player
					...
					f = ! CONFIG.isFilterListHasBiome(biome.getBiomeName())
							? getCloudDensityThreshold(densityByWeather, biome.getRainfall())
				...
				for (int cy = 0; cy < height; cy++) {
					_cloudData[cx][cy][cz] = getCloudSampleProxy(world, sx, sz, 0, time, steps, cx, cy, cz) * densityMultiplier > f && (
							...
							! isEnableTerrainDodge || world.getLight(new BlockPos(
									bx,
